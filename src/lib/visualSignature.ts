...
function getCropBounds(metadata: sharp.Metadata, detection: DetectionCrop) {
...
function buildTextureFeatures(raw: Buffer, size: number) {
...
function buildEdgeFeatures(raw: Buffer, size: number) {
  const features: number[] = []
  const cells = 4
  const cellSize = size / cells
...
function buildColorFeatures(raw: Buffer) {
  const bins = 8
...
async function buildSignatureForOrientation(filePath: string, bounds: NonNullable<ReturnType<typeof getCropBounds>>, mirror: boolean) {
...
async function buildVisualDescriptor(filePath: string | null, detection: DetectionCrop, mirror = false): Promise<VisualDescriptor | null> {
...
function toPercentSimilarity(value: number | null) {
  return value === null ? null : Math.max(0, Math.min(1, value))
}

function compareDescriptors(left: VisualDescriptor | null, right: VisualDescriptor | null) {
...
export async function buildVisualSignature(filePath: string | null, detection: DetectionCrop, mirror = false) {
  if (!filePath || !hasUsableBox(detection)) return null
